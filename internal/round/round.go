...
// IsVerifiable reports whether a round has stopped receiving updates and its
// state can be used for a lineage verdict.
func (s *Service) IsVerifiable(id string) (bool, error) {
	r, err := s.store.GetRound(id)
	if err != nil {
		return false, err
	}
	return r.State == model.RoundStateReceiving || r.State == model.RoundStateValidating || r.State == model.RoundStateAggregable || r.State == model.RoundStateSealed, nil
}
